...
function getAnalyzedClips(manifest) {
    return manifest.clips.filter((clip) => "analysis" in clip && clip.analysis !== undefined);
}
// =============================================================================
// SEARCH HELPERS
// =============================================================================
function searchClips(clips, options) {
    let results = clips;
    ...
    if (options.motionIntensity) {
        results = results.filter((clip) => safeStr(clip.analysis?.motionIntensity).toLowerCase() === options.motionIntensity.toLowerCase());
    }
    // Audio filters
...
async function selectClipForSlot(allClips, requirements) {
...
server.registerTool("get_clip_details", {
...
server.registerTool("find_edit_compatible_clips", {
...
server.registerTool("get_clip_json", {
    title: "Get Clip JSON",
...
server.registerTool("list_sequence_types", {
    ...
    const output = sequenceTypes.map(st => `**${st.name}**\n${st.description}\n\nParameters:\n${Object.entries(st.params).map(([k, v]) => `- ${k}: ${v}`).join('\n')}`).join('\n\n---\n\n');
    return {
        content: [
            {
                type: "text",
...
server.registerTool("build_sequence", {
...
Returns JSON with filled slots, gaps (unfilled slots), and total duration.`,
    ...
    for (const mood of moods) {
        for (let i = 0; i < clipsPerMood; i++) {
            ...
            if (aspectRatio)
                requirements.aspectRatio = aspectRatio;
            if (mustStartClean && isFirst)
                requirements.mustStartClean = true;
            if (mustEndClean && isLast)
                requirements.mustEndClean = true;
...
server.resource("manifest", "veo://manifest", {
    title: "Full Manifest",
